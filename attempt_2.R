...
# Obtain title. If title occurence <7, than assign as 'rare'
...
glm.survive <- glm(Survived~Pclass+age_bracket+Sex+SibSp+Parch+title_agg,
                         data=train,
                         family="binomial")
...
test <- left_join(test,title_occurence,by="title") %>%
  mutate(title_agg = ifelse(n<7,"rare",title))
md.pattern(test)
# drop row without fare
test.age <- dplyr::select(test,Pclass,Sex,SibSp,Parch,Age,Fare)
age.imputed <- mice(test.age,m=5,maxit=50,method='pmm',seed=500)
test$age_imputed <- complete(age.imputed)$Age
test$age_bracket <- sapply(test$age_imputed,age_bracket_function)
...
write.csv(output,"output_log2.csv", row.names = FALSE)
